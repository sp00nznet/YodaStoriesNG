# Palette data from goda-stories (BGRA format)
$paletteBytes = @(
...
# Convert BGRA to ARGB (0xFFRRGGBB)
$result = @()
for ($i = 0; $i -lt 256; $i++) {
    $idx = $i * 4
    $b = $paletteBytes[$idx]
    $g = $paletteBytes[$idx + 1]
    $r = $paletteBytes[$idx + 2]
...
# Output C# array
Write-Output "// Palette from goda-stories project (correct Yoda Stories palette)"
